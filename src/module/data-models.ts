...
/** Shared interface for all ODD item types — name is on the Document itself. */
export interface OddItemBase {
  /** Tags / modifier pills on the item. */
  notes: string[];
}
...
/** Schema for a single hand configuration (1h or 2h stats). */
function weaponHandSchema() {
  return new SchemaField({
    tempos: new NumberField({
      required: true, integer: true,
      min: WEAPON_TEMPO_MIN, max: WEAPON_TEMPO_MAX, initial: 5,
    }),
    distance: new StringField({
      required: true, initial: "M",
      choices: Object.keys(WEAPON_DISTANCE),
    }),
    accuracy: new StringField({
      required: true, initial: "d6",
      choices: Object.keys(DICE_TYPES),
    }),
  ...
  static override defineSchema() {
    return {
      ...
      hands: new StringField({
        required: true, initial: "versatile",
        choices: Object.keys(WEAPON_HANDS),
      }),
...
/** TypeScript shape of a weapon's `system` object. */
